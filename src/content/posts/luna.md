...
借り手という表現でわかりづらかったけど、要は`LUNA`を担保に`UST`を借りている人。つまり`UST`を保持している人が借り手か。
なので`LUNA`を保持していると`bLUNA`が発行されステーキングで利息が入ってくるけど、USTを保持してる人は本来もらえるはずだったステーキングの利息が入ってこない。

このユーザー目線の仕組みはわかるけど、Anchor目線の構造が全然ピンときていない。
LUNAやETHをユーザーから受け取り、`bLUNA`, `bETH`を発行しそれらをステーキング。
